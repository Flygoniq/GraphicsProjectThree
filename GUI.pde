...
void keyPressed()  // executed each time a key is pressed: sets the Boolean "keyPressed" till it is released   
    ...
    if(key=='5') ;
    if(key=='6') ;
    if(key=='7') {println(_hipAngle);println(_ohipAngle);}
    if(key=='8') ;
    if(key=='9') ;
    if(key=='0') ; 
    if(key=='-') ;
    ...
    if(key=='a') _rH=200; _rK=20; _rA=20; _rE=25; _rB=15; _rT=5;; 
    if(key=='b') _rH=100; _rK=50; _rA=20; _rE=25; _rB=15; _rT=5;; 
    ...
    if(key=='s') println("H: " + _H.x + " R: " + _B.x + " L: " + _OB.x);
...
void mouseDragged() // executed when the mouse is dragged (while mouse buttom pressed)
  ...
  if (keyPressed) {
      ...
      if (key=='\\') _ohipAngle-=PI*float(mouseX-pmouseX)/width;  // adjust current frame   
...
String title ="Tango Dancer: Project 3 Part A: caplet ",            name ="Student: Alan JIANG",
